...
class FilterService:
    ...
    async def store_filter(self, user_id: int, user_filter: int):
        ...
        await self.filter_repo.store_filter(user_id, user_filter)
    ...
    async def store_filters(self, user_id, user_filters: list[int]):
        ...
        stored_filters = await self.filter_repo.get_all_filters_user(user_id)
        ...
        await self.filter_repo.store_filters(user_id, new_filters)
    ...
    async def delete_filter(self, user_id, tag_id):
        ...
        if not user:
            raise UserNotFoundException
        await self.filter_repo.delete_filter_user(user_id, tag_id)
    ...
    async def get_all_filters_user(self, user_id) -> list[Tag]:
        ...
        if not user:
            raise UserNotFoundException
        return await self.filter_repo.get_all_filters_user(user_id)
